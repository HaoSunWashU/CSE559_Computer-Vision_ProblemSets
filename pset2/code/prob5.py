...
def ntod(nrm, mask, lmda):
    ...
    ny = nrm[:, :, 1]
    nz = nrm[:, :, 2]
    z0 = np.zeros(mask.shape)
    W = np.where(mask!=0, nz*nz, 0)
    print(W.shape)

    # create gx and gy with H*W
    gx = np.where(mask!=0, -nx/nz, 0)
    print(gx.shape)
    gy = np.where(mask!=0, -ny/nz, 0)
    ...
    for k in range(100):
        Qpk = conv2(conv2(pk, fx, "same") * W, -fx, "same") + conv2(conv2(pk, fy, "same") * W, -fy, "same") + lmda * (
        conv2(conv2(pk, fr, "same"), fr, "same"))
        ak = np.sum(rk * rk) / np.sum(pk * Qpk)
        Z = Z + ak * pk
        r = rk
        rk = rk - ak * Qpk
        bk = np.sum(rk * rk)/np.sum(r * r)
        pk = rk + bk*pk
        k = k + 1
