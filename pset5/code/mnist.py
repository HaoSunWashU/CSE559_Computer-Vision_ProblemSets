...
# Init Weights
def xavier(shape):
    sq = np.sqrt(3.0/np.prod(shape[:-1]))    # prod() compute the product of array elements here the result is 28*28
    return np.random.uniform(-sq,sq,shape)   # Draw samples from a uniform distribution.
